...
def get_all_crm_clients(id_empresa=None):
    query = f"""
        WITH resumen AS (
            SELECT 
                id_cliente,
...
def get_crm_client_by_id(id_usuario, id_empresa=None):
    query = f"""
        WITH resumen AS (
            SELECT 
                id_cliente,
    ...
    params = (
        id_usuario, id_empresa, id_empresa,
        id_usuario, id_empresa, id_empresa,
        id_usuario,
        CLIENT_ROLE_ID,
        id_empresa, id_empresa
...
def get_client_transactions(id_usuario, id_empresa=None, limit=50):
    query = f"""
        SELECT
            nro_transaccion,
            tipo_transaccion,
        ...
        ORDER BY 
            fecha_hora DESC,
            nro_transaccion DESC
        LIMIT %s
    """

    return db.execute_query(
        query,
        (id_usuario, id_empresa, id_empresa, limit),
        fetchall=True
    )


def get_crm_general_stats(id_empresa=None):
    query = f"""
        SELECT
        ...
        INNER JOIN {Config.SCHEMA}.{T_ROL} r 
            ON r.id = u.id_rol
        LEFT JOIN {Config.SCHEMA}.{T_TRANSACCION} t 
