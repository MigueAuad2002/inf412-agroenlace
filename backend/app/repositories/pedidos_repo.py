...
def get_catalogo_insumos(id_empresa):
    query = f"""
...
def verificar_stock(id_producto):
...
def insert_transaccion_cabecera(tipo_transaccion, monto_total, id_cliente, id_supervisor_admin, id_empresa):
    query = f"""
...
def insert_detalle_venta(cantidad, precio_venta, nro_transaccion, id_producto):
    query = f"""
        INSERT INTO {Config.SCHEMA}.detalle_venta 
        (cantidad, precio_venta, nro_transaccion, id_producto)
        VALUES (%s, %s, %s, %s)
    """
    return db.execute_query(query, (cantidad, precio_venta, nro_transaccion, id_producto), commit=True)

def update_stock_bodega(cantidad, id_producto):
    query = f"""
        UPDATE {Config.SCHEMA}.bodega 
        SET stock_actual = stock_actual - %s 
        WHERE id_producto = %s
    """
    return db.execute_query(query, (cantidad, id_producto), commit=True)

def get_historial_pedidos(id_cliente):
...
def get_detalle_pedido(nro_transaccion):
