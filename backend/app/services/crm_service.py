...
from ..repositories import crm_repo
...
def _get_id_empresa_from_payload(payload_token):
    ...
    return (
        payload_token.get("id_empresa")
        or payload_token.get("empresa")
        or payload_token.get("company_id")
    )
...
def classify_client(cliente):
    ...
    if total_transacciones == 0:
        if dias_creacion is not None and dias_creacion <= 30:
            return "NUEVO"
        return "INACTIVO"

    if dias_ultima is not None and dias_ultima > 90:
        return "INACTIVO"

    if dias_creacion is not None and dias_creacion <= 30 and total_transacciones <= 1:
        return "NUEVO"

    if monto_total >= 10000 or total_transacciones >= 8:
        return "VIP"

    if total_transacciones >= 4:
        return "FRECUENTE"
...
def get_clients_list(categoria=None, search=None, estado=None, payload_token=None):
    try:
        ...
        result = crm_repo.get_all_crm_clients(id_empresa=id_empresa)
        ...
        if categoria:
            ...
            clientes = [
                c for c in clientes
                if str(c.get("categoria_cliente")).upper() == categoria
            ]
        ...
        if estado:
            ...
            clientes = [
                c for c in clientes
                if str(c.get("estado_cuenta")).upper() == estado
            ]
        ...
        clientes = sorted(
            clientes,
            key=lambda c: (
                c.get("prioridad_categoria", 99),
                -_to_float(c.get("monto_total"))
            )
        )

        return {
            "success": True,
            "message": "Clientes CRM obtenidos exitosamente.",
            "data": clientes,
            "list_clientes": clientes,
            "stats": _build_stats_from_clients(clientes),
            "categorias": CATEGORIAS_CRM,
        }, 200

    except Exception as e:
        return {
            "success": False,
            "message": f"Error al obtener clientes CRM: {str(e)}"
        }, 500
...
def get_client_detail(id_usuario, payload_token=None):
    try:
        ...
        result = crm_repo.get_crm_client_by_id(
        ...
        trans_result = crm_repo.get_client_transactions(
            id_usuario=id_usuario,
            id_empresa=id_empresa,
            limit=20
        )

        trans_rows = _rows_to_dicts(trans_result)
        ...
        return {
            "success": True,
            "message": "Detalle de cliente obtenido exitosamente.",
            "data": cliente,
            "cliente": cliente,
        }, 200
...
def get_client_transactions_service(id_usuario, payload_token=None):
    try:
        ...
        result = crm_repo.get_client_transactions(
            id_usuario=id_usuario,
            id_empresa=id_empresa,
            limit=50
        )
        ...
        return {
            ...
            "data": data,
            "list_transacciones": data,
        }, 200

    except Exception as e:
        return {
            "success": False,
            "message": f"Error al obtener transacciones del cliente: {str(e)}"
        }, 500
...
def get_crm_stats(payload_token=None):
    try:
        ...
        result = crm_repo.get_crm_general_stats(id_empresa=id_empresa)
        rows = _rows_to_dicts(result)

        general_stats = rows[0] if rows else {}

        clientes_result = crm_repo.get_all_crm_clients(id_empresa=id_empresa)
        clientes_rows = _rows_to_dicts(clientes_result)
        ...
        return {
            "success": True,
            "message": "Estadísticas CRM obtenidas exitosamente.",
            "data": response,
            "stats": response,
        }, 200
        ...
        return {
            "success": False,
            "message": f"Error al obtener estadísticas CRM: {str(e)}"
        }, 500
